...
function 確定母們(上字音韻地位, 下字音韻地位) {
  const 上字母 = 上字音韻地位.母;
  const 被切字母們 = null; // TODO: 處理類隔：上字母到被切字母們.get(上字母);
  if (被切字母們 == null) {
    const 被切字母 = 上字母;
...
export function pyanxchet(上字, 下字, 上字音韻地位, 下字音韻地位) {
  ...
  logger.log(`${上字}${下字}切`);
  logger.log(`${上字}: ${上字音韻地位.表達式}`);
  logger.log(`${下字}: ${下字音韻地位.表達式}`);
  //${生成擬音(上字音韻地位)}
  ...
  if (被切字等們.length > 1) { // 不能確定等的情況
    if (被切字母們.length === 1) {
      const 母對應的等們 = 母到對應的等們.get(首個被切字母);
    ...
    for (const 被切字重紐 of 被切字重紐們) {
      for (const 被切字等 of 被切字等們) {
        for (const 被切字開合 of 被切字開合們) {
          const 當前音韻地位 = new Qieyun.音韻地位(被切字母, 被切字開合, 被切字等, 被切字重紐, 被切字韻, 被切字聲);
          if (當前音韻地位.代表字 == null) {
            logger.log(`規則 10.1: 經查詢可知，音韻地位${當前音韻地位.描述}無字，故排除`);
          } else {
            被切字音韻地位們.push(當前音韻地位);
          }
        }
      }
    }
  ...
  if (被切字音韻地位們.length === 0) {
    ...
    const 當前音韻地位反切 = 當前音韻地位.反切(null) == null ? '' : `, ${當前音韻地位.反切(null)}切`;
    logger.log(`預測被切字音韻地位: ${當前音韻地位.描述}(${當前音韻地位.代表字}${當前音韻地位反切})${生成擬音(當前音韻地位)}`);
  } else {
    let i = 1;
    for (const 當前音韻地位 of 被切字音韻地位們) {
      const 當前音韻地位反切 = 當前音韻地位.反切(null) == null ? '' : `, ${當前音韻地位.反切(null)}切`;
      logger.log(`預測被切字音韻地位 ${i++}: ${當前音韻地位.描述}(${當前音韻地位.代表字}${當前音韻地位反切})${生成擬音(當前音韻地位)}`);
    }
  }
